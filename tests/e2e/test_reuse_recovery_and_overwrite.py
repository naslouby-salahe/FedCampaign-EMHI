...
def test_preprocess_cli_selects_dataset_and_states_ownership() -> None:
    selected = runner.invoke(application, ["preprocess", "edge-iiotset"])
    ...
    both = runner.invoke(application, ["preprocess"])
    assert both.exit_code == 0
    assert DatasetName.TON_IOT_NETWORK.value in both.stdout
    assert DatasetName.EDGE_IIOTSET.value in both.stdout
    overwritten = runner.invoke(application, ["preprocess", "edge-iiotset", "--overwrite"])
