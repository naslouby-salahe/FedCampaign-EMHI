...
def test_all_seven_states_are_declared_byte_exact() -> None:
    labels = {state.label for state in ALL_REGISTRY_STATES}
    assert labels == {
        "SUPPORTED",
        "PARTIALLY_SUPPORTED",
        "MECHANISM_ONLY",
...
def test_not_tested_reserved_for_eligibility_failure() -> None:
    assert not_tested_requires_eligibility_failure(NOT_TESTED, True) is True
    assert not_tested_requires_eligibility_failure(NOT_TESTED, False) is False
    assert not_tested_requires_eligibility_failure(SUPPORTED, False) is True
...
def test_compute_registry_state_decision_order() -> None:
    ...
    assert compute_registry_state(**base) is SUPPORTED
    assert compute_registry_state(**{**base, "eligibility_failed": True}) is NOT_TESTED
    assert compute_registry_state(**{**base, "null_observation": True}) is NULL_RESULT
    assert compute_registry_state(**{**base, "gates_pass": False}) is NOT_SUPPORTED
    assert compute_registry_state(**{**base, "mechanism_only_evidence": True}) is MECHANISM_ONLY
    assert compute_registry_state(**{**base, "conditionality_declared": True}) is CONDITIONAL
    assert compute_registry_state(**{**base, "partial_support": True}) is PARTIALLY_SUPPORTED
