...
def edge_iiotset_ground_truth(
    ...
    normalized_benign_type = benign_attack_type.strip().lower()
    if binary_label == 0 and normalized_type.lower() == normalized_benign_type:
        return GroundTruthLabel(
            classification=GroundTruthClass.BENIGN,
            attack_type=normalized_type,
            is_ambiguous=False,
        )
    if binary_label == 1 and normalized_type.lower() != normalized_benign_type:
        return GroundTruthLabel(
