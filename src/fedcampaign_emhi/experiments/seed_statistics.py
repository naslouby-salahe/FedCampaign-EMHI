...
def materialize_seed_statistics(
    ...
    layout = build_artifact_layout(loaded, repository)
    root = layout.experiment_outputs_root(experiment_name)
    summary_root = root / ArtifactPathSegment.METRICS / ArtifactPathSegment.SEED_SUMMARIES
    summary_paths = tuple(sorted(summary_root.glob(f"**/{ArtifactFileSuffix.JSON_GLOB}")))
    ...
    for index, (method_name, records) in enumerate(method_groups):
        ...
        payload: YamlNode = {
            ...
            ArtifactMetadataKey.METRIC_NAME: MetricName.STRICT_ODI_RATE,
        ...
        record = StatisticalRecord(
            ...
            metric_name=MetricName.STRICT_ODI_RATE,
        ...
        path = (
            ...
            / "seed-level"
            / ArtifactFilenamePattern.METHOD_METRIC_JSON.format(
                method=method_artifact_stem(method_name), metric=MetricName.STRICT_ODI_RATE
            )
        )
...
def materialize_not_tested_primary_holm_statistic(
    ...
    payload: YamlNode = {
        ...
        ArtifactMetadataKey.METRIC_NAME: metric_name,
    ...
    path = (
        ...
        / KnownArtifactOutputFilename.PRIMARY_HOLM_NOT_TESTED
    )
    write_atomic_json(
        path,
...
def _load_seed_summaries(
    ...
    summary_root = root / ArtifactPathSegment.METRICS / ArtifactPathSegment.SEED_SUMMARIES
    paths = tuple(sorted(summary_root.glob(f"**/{ArtifactFileSuffix.JSON_GLOB}")))
...
def _confirmatory_method_summaries(
    ...
    return tuple(
        ...
        and record.metric_name == MetricName.STRICT_ODI_RATE
    )
...
def _materialize_paired_confirmatory_odi_contrast(
    ...
    if not complete or not matched_eligible:
        ...
        path = (
            ...
            / ArtifactPathSegment.STATISTICS
            / ArtifactPathSegment.TESTS
            / ArtifactFilenamePattern.HYPOTHESIS_JSON.format(
                hypothesis=_hypothesis_artifact_stem(hypothesis_identifier)
            )
        )
        write_atomic_json(
            path,
...
def materialize_strong_local_odi_statistic(
    ...
    payload: YamlNode = {
        ...
        ArtifactMetadataKey.METRIC_NAME: MetricName.STRONG_LOCAL_STRICT_ODI_RATE,
        "method_name": MethodName.FULL_FEDCAMPAIGN_EMHI.value,
    ...
    record = StatisticalRecord(
        hypothesis_identifier=hypothesis_identifier.value,
        metric_name=MetricName.STRONG_LOCAL_STRICT_ODI_RATE,
        method_name=MethodName.FULL_FEDCAMPAIGN_EMHI.value,
        independent_unit_count=len(shifted),
        estimate=estimate,
        raw_p_value=raw_p_value,
    ...
    path = (
        ...
        / ArtifactPathSegment.STATISTICS
        / ArtifactPathSegment.TESTS
        / KnownArtifactOutputFilename.STRONG_LOCAL_ODI_ABOVE_MINIMUM
    )
    write_atomic_json(
        path,
        cast(YamlNode, record.model_dump(mode="json")),
...
def materialize_benign_common_mode_statistic(
    ...
    payload: YamlNode = {
        ...
        ArtifactMetadataKey.METRIC_NAME: MetricName.FALSE_CAMPAIGN_REDUCTION,
        "method_name": MethodName.FULL_FEDCAMPAIGN_EMHI.value,
        "independent_unit_count": len(differences),
        "estimate": estimate,
        "raw_p_value": raw_p_value,
    ...
    record = StatisticalRecord(
        hypothesis_identifier=PrimaryHolmHypothesis.COMMON_MODE_FALSE_CAMPAIGN_REDUCTION.value,
        metric_name=MetricName.FALSE_CAMPAIGN_REDUCTION,
        method_name=MethodName.FULL_FEDCAMPAIGN_EMHI.value,
        independent_unit_count=len(differences),
        estimate=estimate,
        raw_p_value=raw_p_value,
    ...
    path = (
        ...
        / ArtifactPathSegment.STATISTICS
        / ArtifactPathSegment.TESTS
        / KnownArtifactOutputFilename.COMMON_MODE_FALSE_CAMPAIGN_REDUCTION
    )
    write_atomic_json(
        path,
        cast(YamlNode, record.model_dump(mode="json")),
    ...
    with ProcessPoolExecutor(
        ...
        for factor, seed, outcome in outcomes:
            ...
            path = (
                ...
                / ArtifactPathSegment.COUNT_STRESS
                / f"factor-{factor}"
                / ArtifactFilenamePattern.SEEDED_JSON.format(seed=seed)
            )
            write_atomic_json(path, cast(YamlNode, record.model_dump(mode="json")), staging)
...
def materialize_benign_common_mode_positive_power_measurement(
    ...
    path = (
        root
        / ArtifactPathSegment.DIAGNOSTICS
        / ArtifactPathSegment.POSITIVE_POWER
        / KnownArtifactOutputFilename.MEASUREMENT
    )
    write_atomic_json(
        path,
        cast(YamlNode, record.model_dump(mode="json")),
